...
class EEGMMIDBDataset(Dataset):
    ...
    def _load_single_run(self, subj_id, run):
        ...
        for onset, _, code in events:
            # Find the string name (T0, T1, or T2) for this numerical code
            label_name = None
            for k, v in event_id.items():
                if v == code:
                    # k might look like "T1" or "stimulus/T1"
                    label_name = k.split("/")[-1] 
                    break
        
            # 3. ONLY proceed if the label is in your CLASS_MAP
            if label_name in CLASS_MAP:
                # Use the CLASS_MAP to get 0 or 1
    ...
    def __getitem__(self, idx):
        # Original shape: (22, 1000)
        full_segment = self.data[idx] 
        
        # Split into two chunks of 500 samples each
        # Resulting shape: (2, 22, 500)
        chunk1 = full_segment[:, :500]
        chunk2 = full_segment[:, 500:1000]
        
        # Stack them on a new dimension
        segment = torch.stack([chunk1, chunk2], dim=0) 
